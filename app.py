...
# --- TASK 2: PROTECTED ROUTE INTERCEPTION ---
@app.route('/protected')
async def protected():
    """Protected page - only authenticated users can access"""
    user = await auth0.get_user(g.store_options)
    
    # Custom Guard Clause: If user session is empty, redirect them to the login endpoint
    if not user:
        return redirect(url_for('login'))
...
@app.route('/protected')
async def protected():
    """Protected Page - Only authenticated users can access this resource."""
    # Query the SDK to see if a valid session exists
    user = await auth0.get_user(g.store_options)
    
    # GUARD CLAUSE: If the user is not logged in, redirect them to the login page
    if not user:
        return redirect(url_for('login'))
    
    # If authenticated, render the secure dashboard layout and inject user data
    return render_template('protected.html', user=user)   